...
if($do=='edit') {
$stmt= $con->prepare("SELECT * FROM website_videos WHERE id=?");
...
if(empty($error)){
    ...
    if(!empty($_FILES['image']['name'])){
     ...
     $stmt=$con->prepare("UPDATE  website_videos SET link=?,descryption=?, img=? WHERE id=?");     
     $stmt->execute(array($link,$descryption,$image,$id));
    }else{
        $stmt=$con->prepare("UPDATE  website_videos SET link=? ,descryption=? WHERE id=?");     
        $stmt->execute(array($link,$descryption,$id));
    }
    header("Location:website_showvideo.php?mssg=".urlencode("Done Editing Video"));
      exit();
}
}
                              ...
                              <form class="new-added-form" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-xl-6 col-lg-6 col-12 form-group">
                                            <label>Link *</label>
                                            <textarea  name="link" class="form-control" required style=""><?php echo $subjects['link']; ?></textarea>
