...
if(empty($error)){
    ...
    if(!empty($_FILES['image']['name'])){
        ...
        $stmt=$con->prepare("INSERT INTO website_teacher (title,name,img) VALUES(:title,:name,:img)");
        $stmt->execute(array('title'=>$title,'name'=>$name,'img'=>$image));

    }else{
        $stmt=$con->prepare("INSERT INTO website_teacher (title,name) VALUES(:title,:name)");
        $stmt->execute(array('title'=>$title,'name'=>$name));
    }
        header("Location:website_showteacher.php?mssg=".urlencode("Done Adding Teacher"));
        exit();
}
}
