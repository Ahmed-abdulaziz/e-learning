...
if(isset($_POST['edit']))
    ...
    $stmt=$con->prepare("DELETE FROM homeworkquestions WHERE homeworkid=?");
    $stmt->execute(array($id));
    ...
    if(!empty($mcq)){
          ...
          foreach ($mcq as $q) {
                ...
                if($q != 0){
                    ...
                    $stmt->execute(array('homeworkid'=> $id ,'questionid'=>$q,'type'=>'0' , 'degree'=>1));
                    $total = $total + 1;
  ...
  if(!empty($essay)){
        ...
        foreach (array_combine($essay, $essaydegree) as $q => $degree) {
                ...
                if($q != 0){
                    ...
                    $stmt=$con->prepare("INSERT INTO homeworkquestions (homeworkid,questionid,type , degree)
                    VALUES(:homeworkid,:questionid,:type ,:degree)");
                    $stmt->execute(array('homeworkid'=> $id ,'questionid'=>$q,'type'=>'1' , 'degree'=>$degree));
                    $total = $total + $degree;
    ...
    foreach($givens as $given){
                ...
                $stmt=$con->prepare("INSERT INTO homeworkquestions (homeworkid,questionid,type , degree)
                VALUES(:homeworkid,:questionid,:type ,:degree)");
                $stmt->execute(array('homeworkid'=> $id ,'questionid'=>$given,'type'=>'2' , 'degree'=>$degree));
...
if(empty($err)){
      $stmt=$con->prepare("UPDATE  homeworks SET name=?  , degree = ? , answer = ?  WHERE id=?");     
      $stmt->execute(array($name, $total , $Mimage ,$id));
    header("Location:homeworks.php?mssg=".urlencode("Done Editing homework"));
      exit();
}
}
...
<head>
    ...
    <title>E-Learning System | Editing Homeworks</title>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form class="new-added-form" method="post" enctype="multipart/form-data">
                                    ...
                                    <!--        <label>Time</label>-->
                                    <!--        <input type="number" placeholder="" class="form-control" value="<?php echo $exams['timer']; ?>" name="timer" required>-->
                                        ...
                                        <table class="table table-striped display data-table  text-center overflow-auto h-75" id="myTable">
                                                      ...
                                                      <tbody>
                                                         ...
                                                         foreach($mcq as $q){
                                                            ...
                                                            $stmt=$con->prepare("SELECT questionid,degree FROM homeworkquestions WHERE homeworkid = ? AND questionid=? AND type = ?");
                                                        ...
                                                        <tr>
                                                         ...
                                                         <?php }
                                                          ...
                                                          <td><?php if(!empty($lesson['name'])){echo $lesson['name']; } ?></td>
                                                                   ...
                                                                   <input class="form-control" hidden value="<?php echo $idq['degree'];?>" name="<?php echo $essayname?>degree[]" type="number" placeholder="Enter degree of question">
                                                     ...
                                                     <?php 
                                                        ...
                                                        <tr>
                                                          <th scope="row">
                                                              ...
                                                              <input type="checkbox" class="form-check-input" name="<?php echo $name?>[<?php echo $i?>]" value="<?php echo $q['id'];?>" <?php if ($q['id']==$idq['questionid']){ echo "checked";}?>>
                                                            </th>
                                                          <td class="w-50">
                                                              <?php
                                                                if(!empty($q['text'])){
...
<!--
                <footer class="footer-wrap-layout1">
                    <div class="copyright">© Copyrights <a href="#">Mr. Ibrahim Gad</a> 2019. All rights reserved. Designed by <a href="#">Mohamdy</a></div>
                </footer>
-->
            </div>
        </div>
