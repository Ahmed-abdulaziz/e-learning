...
if($do=='edit') {
$stmt= $con->prepare("SELECT * FROM website_subject WHERE id=?");
...
if(empty($error)){
    ...
    if(!empty($_FILES['image']['name'])){
     ...
     $stmt=$con->prepare("UPDATE  website_subject SET title=?,descryption=?,subject=? , img=? WHERE id=?");     
     $stmt->execute(array($title,$descryption,$subject,$image,$id));
    }else{
        $stmt=$con->prepare("UPDATE  website_subject SET title=?,descryption=?,subject=? WHERE id=?");     
        $stmt->execute(array($title,$descryption,$subject,$id));
    }
    header("Location:website_showsubject.php?mssg=".urlencode("Done Editing Subject"));
      exit();
}
}
...
<head>
    ...
    <title>E-Learning System | Editing Subject</title>
                              ...
                              <form class="new-added-form" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                     ...
                                     <div class="col-xl-6 col-lg-6 col-12 form-group">
                                            ...
                                            <input type="text" placeholder="" class="form-control" name="descryption" value="<?php echo $subjects['descryption']; ?>" required>
                                        </div>
                                           <div class="col-xl-6 col-lg-6 col-12 form-group">
                                            <label>Subject *</label>
                                            <input type="text" min="0" placeholder="" class="form-control" name="subject" value="<?php echo $subjects['subject']; ?>" required>
                                        </div>
